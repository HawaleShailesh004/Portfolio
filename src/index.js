...
document.querySelectorAll('a,button,.ci,.acard,.wc,.ecd,.pla,.hpt,.spp').forEach(el => { el.addEventListener('mouseenter', () => document.body.classList.add('ch')); el.addEventListener('mouseleave', () => document.body.classList.remove('ch')); });
...
// LOADER
const lt = document.getElementById('lt'), lb = document.getElementById('lb'), loader = document.getElementById('loader');
const ls = ' loading portfolio...'; let li = 0;
function tl() { if (li < ls.length) { lt.textContent += ls[li++]; setTimeout(tl, 55); } }
tl(); setTimeout(() => lb.style.width = '100%', 200);
setTimeout(() => { loader.classList.add('hide'); document.body.classList.remove('loading'); setTimeout(() => loader.style.display = 'none', 800); }, 1900);

// TYPEWRITER
const phrases = ['building AI-powered systems.', 'shipping production backends.', 'MERN + LLMs in the real world.', 'turning ideas into live products.'];
const twEl = document.getElementById('tw');
let pi = 0, ci = 0, del = false, started = false;
function tw() {
  if (!started) return;
  const w = phrases[pi];
  if (!del) { twEl.textContent = w.slice(0, ++ci); if (ci === w.length) { del = true; setTimeout(tw, 2000); return; } setTimeout(tw, 60); }
  else { twEl.textContent = w.slice(0, --ci); if (ci === 0) { del = false; pi = (pi + 1) % phrases.length; setTimeout(tw, 400); return; } setTimeout(tw, 35); }
}
setTimeout(() => { started = true; tw(); }, 2600);
...
// MOBILE MENU
const hb = document.getElementById('hb'), mm = document.getElementById('mm'), mc = document.getElementById('mc');
hb.addEventListener('click', () => { mm.classList.toggle('open'); hb.classList.toggle('open'); });
mc.addEventListener('click', () => { mm.classList.remove('open'); hb.classList.remove('open'); });
document.querySelectorAll('.ml').forEach(l => l.addEventListener('click', () => { mm.classList.remove('open'); hb.classList.remove('open'); }));

// REVEAL
const rvEls = document.querySelectorAll('.rv,.rl,.rr');
const ro = new IntersectionObserver(e => e.forEach(en => { if (en.isIntersecting) en.target.classList.add('vis'); }), { threshold: .1, rootMargin: '0px 0px -50px 0px' });
rvEls.forEach((el, i) => { const p = el.parentElement; const s = [...p.querySelectorAll('.rv,.rl,.rr')]; el.style.transitionDelay = (s.indexOf(el) * .08) + 's'; ro.observe(el); });

// COUNTERS
const co = new IntersectionObserver(e => e.forEach(en => {
  if (en.isIntersecting) {
    const el = en.target, t = +el.dataset.count, d = 1200, s = performance.now();
    (function tick(n) { const p = Math.min((n - s) / d, 1), ease = 1 - Math.pow(1 - p, 3); el.textContent = Math.floor(ease * t); if (p < 1) requestAnimationFrame(tick); else el.textContent = t; })(s);
    co.unobserve(el);
  }
}), { threshold: .5 });
...
// COPY
function copyTxt(text, el) { navigator.clipboard.writeText(text).then(() => { const f = el.querySelector('.cf'), a = el.querySelector('.cia'); if (f) { f.classList.add('show'); if (a) a.style.opacity = '0'; setTimeout(() => { f.classList.remove('show'); if (a) a.style.opacity = '1'; }, 2000); } }); }

// PARALLAX
const b1 = document.querySelector('.b1'), b2 = document.querySelector('.b2');
window.addEventListener('scroll', () => { const y = scrollY; b1 && (b1.style.transform = `translateY(${y * .15}px)`); b2 && (b2.style.transform = `translateY(${-y * .1}px)`); }, { passive: true });
